      ...
      <div className="mt-6 bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm overflow-hidden select-none">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-5 mb-5">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h2 className="font-heading text-[24px] font-black text-slate-900 m-0 leading-tight">
                Audit history
              </h2>
              <p className="text-[15px] font-medium text-slate-400 m-0 mt-0.5">
                Append-only audit trail — {audit.length} event{audit.length === 1 ? '' : 's'} recorded.
              </p>
            </div>
          </div>

          <span className="text-[14.5px] font-black text-indigo-700 bg-indigo-50 px-3.5 py-1.5 rounded-full border border-indigo-200/80 shadow-2xs">
              ...
              {audit.map((entry) => {
                ...
                return (
                  <tr key={entry.auditId} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    {/* Timestamp */}
                    <td className="py-3.5 px-4 align-top text-right whitespace-nowrap font-semibold text-slate-400 text-[14px]">
