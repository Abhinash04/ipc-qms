...
export function AssignedOfficialDashboard({
  ...
  const draftingCount = myQueries.filter(
    (q) =>
      q.workflowState === WORKFLOW_STATE.DRAFTING ||
      q.workflowState === WORKFLOW_STATE.ASSIGNED,
  ).length;
  const underReviewCount = myQueries.filter(
    (q) => q.workflowState === WORKFLOW_STATE.UNDER_REVIEW,
  ).length;
  const returnedCount = myQueries.filter(
    (q) => q.workflowState === WORKFLOW_STATE.RETURNED_FOR_REVISION,
  ).length;
  ...
  const kpis = [
    ...
    },
    {
      ...
      icon: FileText,
      onClick: () =>
        (paths.MY_WORK || paths.QUERIES) &&
        navigate(paths.MY_WORK || paths.QUERIES),
    },
    {
      ...
      subtextMain:
        returnedCount > 0
          ? `! ${returnedCount} returned for changes`
          : "No returned drafts",
