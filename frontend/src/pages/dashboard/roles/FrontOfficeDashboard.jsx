...
export function FrontOfficeDashboard({ currentUser, queries, auditEvents }) {
  ...
  const pendingDispatch = queries.filter(
    (q) => q.workflowState === WORKFLOW_STATE.PENDING_DISPATCH,
  );
  const openQueries = queries.filter(
    (q) =>
      q.workflowState !== WORKFLOW_STATE.CLOSED &&
      q.workflowState !== WORKFLOW_STATE.PENDING_VERIFICATION,
  );
  const closedToday = countSince(auditEvents, AUDIT_EVENT.QUERY_CLOSED, 1);
  const receivedThisWeek = countSince(
    auditEvents,
