...
export function ReviewerDashboard({ currentUser, queries, workflowSteps, auditEvents }) {
  ...
  const approvedToday = countSince(auditEvents, AUDIT_EVENT.REVIEW_APPROVED, 1) + countSince(auditEvents, AUDIT_EVENT.FINAL_APPROVAL_GRANTED, 1);
  const rejectedToday = countSince(auditEvents, AUDIT_EVENT.REVISION_REQUESTED, 1) + countSince(auditEvents, AUDIT_EVENT.FINAL_APPROVAL_REJECTED, 1);

  const recentlyClosed = newestFirst(auditEvents.filter((e) => e.event === AUDIT_EVENT.QUERY_CLOSED))
    .slice(0, 3)
    .map((event) => {
