...
function closureRate(auditEvents, closedQueryIds, from, to = new Date()) {
  const received = auditEvents.filter(
    (e) =>
      e.event === AUDIT_EVENT.QUERY_RECEIVED &&
      new Date(e.at) >= from &&
      new Date(e.at) < to,
  );
  ...
  return {
    percent: Math.round((closed / received.length) * 100),
...
export function DashboardResolutionRate({ visibleAudit, closedQueryIds }) {
  ...
  const rates = [
    {
      label: "This month",
      rate: closureRate(visibleAudit, closedQueryIds, startOfMonth),
    },
    {
      label: "This week",
      rate: closureRate(visibleAudit, closedQueryIds, since(7)),
    },
    {
      label: "Last week",
      rate: closureRate(visibleAudit, closedQueryIds, since(14), since(7)),
    },
  ];
      ...
      <div>
        <div className="flex justify-between items-start mb-6">
          <div className="flex gap-4 items-start">
            ...
            <div>
              <h3 className="font-heading text-[26px] font-black text-slate-900 m-0 leading-tight">
                Resolution rate
              </h3>
          ...
          <div className="relative w-22 h-14 shrink-0 pointer-events-none select-none hidden sm:block">
            ...
            <div className="absolute top-1 right-0 w-18 h-12 bg-white/95 rounded-xl shadow-[4px_4px_8px_#d0d7e5,-4px_-4px_8px_#ffffff] border border-slate-100 p-1.5 flex flex-col justify-between transform rotate-3">
              ...
              <svg width="100%" height="16" viewBox="0 0 50 16" fill="none">
                <path
                  d="M2 14 L14 10 L26 12 L38 4 L46 2"
                  stroke="#6366f1"
                  strokeWidth="2"
                  strokeLinecap="round"
                  fill="none"
