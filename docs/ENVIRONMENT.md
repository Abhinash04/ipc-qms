...
**Joining as a teammate.** Do not copy the host's `backend/.env.local`: it is the host profile, and
on your machine it would start a second mailbox host. Fill in the teammate column above, with your
own Atlas user, and ask the Atlas project owner to add your public IP to Network Access. Then run
`npm run db:check` in `backend/`: it only reads from the database, prints no password, and names the
failing step (env file, URL, DNS, TCP/TLS, access list, credentials). A
`querySrv ECONNREFUSED` is a DNS problem on your machine, not Atlas; the fixes are in the
[root README](../README.md#shared-development-database-mongodb-atlas) under **Cannot connect?**. The
check's last line is a fingerprint of the database; it must match the host's when both run at the
same moment.
...
To check the database connection itself, run `npm run db:check` (section 5, **Joining as a
teammate**). It connects to the database and only reads from it.
