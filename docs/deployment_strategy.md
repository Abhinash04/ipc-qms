...
| Background workers | One in-process timer: the mailbox junk-retention sweep, hourly, `unref`'d, started in `server.js` and stopped first on shutdown (`services/email/mailbox/retention.js`). Off when `MAILBOX_RETENTION_ENABLED=false` and silent under `NODE_ENV=test`. No job queue, cron, `worker_threads` or `child_process`. [Verified, 2026-09-24] |
...
2. Ingestion is **triggered by the browser, not the server**:
   ...
   - **Ingestion has no server-side scheduler.** If nobody is signed in as the NICeMail Front Officer, no mail is ingested. Cron and a job queue are absent, and the one server-side timer that does exist — the hourly junk-retention sweep (`services/email/mailbox/retention.js`) — reads and prunes stored mail but never reaches the mailbox, so it cannot ingest anything. [Verified, 2026-09-24]
...
| 11 | Medium | Ingestion only while the NICeMail Front Officer is signed in to IPC-QMS; limited loaded window (~50 rows [Documented only]) | Keep a session open during office hours; server-side scheduler (code) |
...
| Background workers | Hourly junk-retention sweep, in process | Negligible | Negligible | `unref`'d `setInterval`; no cron, queue or worker threads |
...
| 10 | Email addresses and PII | Inquirer addresses, subjects and bodies are stored in MongoDB. Addresses and subjects **appear in application logs**. Retention now covers **junk only**: mail triaged as junk has its body, HTML and attachment bytes stripped after `MAILBOX_RETENTION_HOURS` (46 by default), leaving an id stub, and every purge writes an `EMAIL_PURGED` audit row naming the sender and subject — so the audit trail deliberately retains the PII the message body loses. Mail that became a case, and the logs, still have **no retention policy**. [Verified, 2026-09-24] | Medium | Restrict log access; define retention for accepted mail and for logs; include logs and the `EMAIL_PURGED` rows in the data-protection assessment |
...
**Missing:** metrics, alerting, log files and rotation, a real readiness probe, a Browser Agent or CDP health check, and a server-side sync scheduler.
...
5. Code hardening:
   ...
   - an optional server-side sync scheduler;
