...
- **MailboxMessage** (`MailboxMessage.js`) — the IPC mailbox: `mailboxMessageId` (unique index), `to` (indexed), `from`, `cc`, `bcc`, `subject`, `body`, `attachments`, `receivedAt`, `ingested` (indexed), `aiSummary`, plus three fields for mail read from a provider rather than deposited locally:
  ...
  - `purgedAt` — set when the retention sweep strips a message's content. Distinct from `removedAt` on purpose: `removedAt` alone means a person deleted it, both set means the machine reclaimed it. A purge empties `body`, `bodyHtml`, `attachments` and `aiSummary`, unlinks the attachment bytes from disk, and sets `removedAt` too so the stub drops out of the inbox scope — but it **keeps `providerMessageId` and `mailboxMessageId`**, because the skip-set a sync builds from stored provider ids is the only thing stopping the message being ingested again. That is why retention strips fields instead of deleting the document, and why a TTL index could not do this job even if these timestamps were `Date`s rather than strings.

    Two windows decide it, both measured from `MailboxTriage.classifiedAt` — first sight, not the message's own date, since `receivedAt` holds the sync time whenever `receivedAtSource === 'sync'`. `MAILBOX_RETENTION_HOURS` (42) takes mail a rule or the model judged junk at confidence ≥ `MAILBOX_JUNK_CONFIDENCE`, or mail a person rejected. `MAILBOX_UNREGISTERED_RETENTION_HOURS` (336) takes anything still unregistered whatever its verdict, and is what actually bounds the collection: a genuine enquiry is pinned at confidence 0 and so can never satisfy the first window at any age. A message that became a `QueryCase`, that somebody accepted, or that somebody rescued is exempt from both, and the veto is re-checked immediately before each write so a case created mid-sweep still saves its message. Every purge writes an `EMAIL_PURGED` audit row carrying the sender, subject and received time, so what was destroyed stays answerable after the body is gone.
